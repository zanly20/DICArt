...
class D3PM(nn.Module):
    def __init__(self, cfg, num_bins=None, T=None, device='cuda'):
        super().__init__()
        self.cfg = cfg
        self.num_bins = num_bins if num_bins is not None else cfg.num_bins
        ...
        self.T = T if T is not None else cfg.diffusion_steps
        self.device = device
        self.mse_weight = 0.8
        self.L1_weight = 0.1
        self.ce_weight = 1
        ...
        if self.cfg.pts_encoder == 'pointnet':
            ...
            self.pts_encoder = Pointnet2ClsMSG(input_channels=3)
        ...
        self.pivotunitvec_est = nn.Sequential(
            nn.Linear(1024, 512),
            self.act,
            nn.Linear(512, 256),
            self.act,
        ...
        self.mean_parts_gt_est = nn.Sequential(
            nn.Linear(1024, 512),
            self.act,
            nn.Linear(512, 256),
            self.act,
        ...
        self.pivotvec_est = nn.Sequential(
            nn.Linear(1024, 512),
            self.act,
            nn.Linear(512, 256),
            self.act,
        ...
        # canonical
        self.normals_canonical = nn.Sequential(
            nn.Linear(1024, 512),
            self.act,
            nn.Linear(512, 256),
            self.act,
    ...
    def extract_pts_feature(self, data):
        pts = data['pts']
        parts_normals_canonical = data['parts_normals_canonical']
        normals_expanded = parts_normals_canonical.unsqueeze(1).repeat(1, pts.shape[1], 1)  # [B, N, 3]
        pts_with_normals = torch.cat([pts, normals_expanded], dim=-1)

        if self.cfg.pts_encoder == 'pointnet':
            return self.pts_encoder(pts_with_normals.permute(0,2,1))
        elif self.cfg.pts_encoder == 'pointnet2':
            return self.pts_encoder(pts_with_normals)
    ...
    def sample(self, pts_feat, steps=1000):
        ...
        d_normals_canonical = self.normals_canonical(pts_feat)
        pivotunitvec = self.pivotunitvec_est(pts_feat)
        heat = self.H_est(pts_feat) 
        pivotvec = self.pivotvec_est(pts_feat)
        mean_parts_gt = self.mean_parts_gt_est(pts_feat)
        ...
        return y_t, d_normals_canonical, pivotunitvec, heat, pivotvec, mean_parts_gt
    ...
    def loss(self, y, pts_feat, data):
        gt_joint_rpy = data['gt_joint_rpy']
        gt_unitvec = data['gt_unitvec']
        gt_heatmap = data['gt_heatmap']
        gt_mean_parts_gt = data['gt_parts']
        gt_normals_canonical = data['gt_parts_normals_canonical']
        es_normals_canonical = data['parts_normals_canonical']
        ...
        loss_canonical = torch.mean(torch.sum(1 - F.cosine_similarity(d_normals_canonical, gt_d_normals_canonical, dim=1))) 
        loss_H = torch.mean(torch.sum(torch.abs(heat - gt_heatmap))) 
        loss_pivotunitvec = torch.mean(torch.sum(1 - F.cosine_similarity(pivotunitvec, gt_unitvec, dim=1)))   # [bs, 3]
        loss_pivotvec = torch.mean(torch.sum(1 - F.cosine_similarity(pivotvec, gt_joint_rpy, dim=1)))   # pivotvec  [bs, 3]
        loss_mean_parts_gt = torch.mean(torch.sum(torch.norm(mean_parts_gt - gt_mean_parts_gt, dim=1, p=2)))
        loss_description = (
            ...
            # f"Canonical Loss Contribution: {loss_canonical:.4f}"
        )
        print('CE loss:', self.ce_weight*ce_loss.item())
        print('L1 loss:', self.L1_weight*L1_loss.item())
        print('vb loss:', self.vb_weight*vb_loss.item())
        print('Normal loss', 0.07*loss_canonical.item())
        print('H loss', loss_H.item())
        print('Pivotunitvec loss', 0.03*loss_pivotunitvec.item())
        print('Pivotvec loss', 0.3*loss_pivotvec.item())
        print('Mean part loss', 0.3*loss_mean_parts_gt.item())
        # total_loss = self.ce_weight*ce_loss  + self.L1_weight*L1_loss + self.vb_weight * vb_loss
        total_loss = self.ce_weight*ce_loss  + self.L1_weight*L1_loss + self.vb_weight * vb_loss + 0.07*loss_canonical + loss_H + 0.03*loss_pivotunitvec + 0.3*loss_pivotvec + 0.3*loss_mean_parts_gt
        print(loss_canonical.item(),loss_H.item(),loss_pivotunitvec.item(),loss_pivotvec.item(),loss_mean_parts_gt.item())
...
class D3PM_Guassion_Flow(nn.Module):
    def __init__(self, cfg, num_bins=None, T=None, device='cuda'):
        super().__init__()
        self.cfg = cfg
        self.num_bins = num_bins if num_bins is not None else cfg.num_bins  # Number of standard tokens (K)
        self.mask_token = self.num_bins  # Define [MASK] as token with index num_bins
        self.total_tokens = self.num_bins + 1  # K+1 (including [MASK])
