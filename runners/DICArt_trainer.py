...
writer = SummaryWriter(log_dir="path/to/your/project/DICArt/log")
debug = True
...
class D3PMTest:
    ...
    def test_step(self, batch_sample):
        ...
        with torch.no_grad():
            pred = self.model.sample(pts_feat)
            ...
            new_matrix = MinkowskiEngine.MinkowskiTensor(pred_matrix, coords=pts_feat.coords)
            if debug:
                print(new_matrix)
...
class D3PMTrainer:
    ...
    def train_step(self, batch_sample):
        ...
        pts_feat = self.model.extract_pts_feature(batch_sample).to(self.device)
        loss,loss_description = self.model.loss(y, pts_feat, batch_sample)
    ...
    def eval_step(self, batch_sample):
        ...
        try:
            with torch.no_grad():
                ...
                pred = self.model.sample(pts_feat, steps=1000)
...
def train_data(cfg, train_loader, val_loader, test_loader,trans_stats):
    ...
    save_dir = "path/to/your/project/DICArt/D3PM_6D"
    ...
    for epoch in range(cfg.n_epochs):
        ...
        writer.add_scalar("loss/total", avg_train_loss.item(), epoch)
