...
def rot_diff_degree(rot1, rot2, chosen_axis=None, flip_axis=False):
    rot_diff = rot_diff_rad(rot1, rot2, chosen_axis=chosen_axis, flip_axis=flip_axis) / np.pi * 180.0
    if rot_diff > 90:
        rot_diff = 180.0 - rot_diff
    return rot_diff
...
def compute_RT_errors(RT_1, RT_2, class_id, synset_names):
    """
    Args:
...
def compute_RT_overlaps(class_ids, gt_RT, pred_RT, synset_names):
    """ Finds overlaps between prediction and ground truth instances.
...
def get_metrics(pose_1, pose_2, class_ids, synset_names, pose_mode, o2c_pose=False):
    assert pose_mode in ['quat_wxyz', 'quat_xyzw', 'euler_xyz', 'euler_xyz_sx_cx', 'rot_matrix'],\
        f"the rotation mode {pose_mode} is not supported!"

    index = get_pose_dim(pose_mode) - 3  # 9 - 3
    ...
    error = compute_RT_overlaps(class_ids, RT_1.cpu().numpy(), RT_2.cpu().numpy(), synset_names)
    rot_error = error[:, 0]
    trans_error = error[:, 1]
